...
% from CTN@KU
% DataDir = 'C:\Users\verena\Matlab codes\Verena';
DataDir = 'C:\Users\verena\Matlab codes\Verena';

DataFiles1 = {
    {        
     'NphR.-Rnone.abf',ONECOLOR;
}}

DataFiles2 = {
    {
    'SwiChR-R1.abf',TWOCOLOR;
}}
...
for ii=1:NumSlice
    ...
    for jj=1:NumExperiment
        ...
        % added draw cumulative distributions (needs stats toolbox)
        subplot(2,2,4)
...
%figure('Name',DataFiles{ii}{jj});
%plot(TAxis,RawData(:,1),'k'); ylabel(RawDataInfo.recChUnits{1});xlabel('time (s)');
%figure('Name',DataFiles{ii}{jj});
%plot(TAxis,RawData(:,1)); ylabel(RawDataInfo.recChUnits{1});xlabel('time (s)');
%hold on;
%plot(TAxis,FilteredEphysData,'r'); ylabel(RawDataInfo.recChUnits{1});xlabel('time (s)');
